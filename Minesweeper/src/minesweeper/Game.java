...
public class Game {
	...
	private boolean playGame() {
		...
		System.out.println("enter row and column number with space to heat that cell");
		System.out.println("enter exit to exit from game ");
		System.out.println("ex: 1 3");
		System.out.println("ex: exit");
		
		while (state == GameState.RUNNING) {
			String input = (rand.nextInt(rows))+" "+ (rand.nextInt(columns)); //scanner.nextLine();
